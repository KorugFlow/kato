...
Функция find ищет паттерн в строке или массиве и возвращает индекс первого совпадения:
...
Параметры:
- `target` - строка или массив для поиска
- `pattern` - паттерн для поиска (строка или символ)

Возвращает: `int` - индекс первого совпадения или -1 если не найдено
...
**ВАЖНО:**
- Функция требует ровно 2 аргумента (target, pattern)
- Возвращает -1 если паттерн не найден
- Поиск чувствителен к регистру

## Структуры

Kato поддерживает структуры для группировки связанных данных:
...
**ВАЖНО:**
- Структуры объявляются вне функций
- Имя структуры 'c' зарезервировано для C interop
- Поля структуры могут быть типов: int, float, char, string
- Нельзя дублировать имена полей в одной структуре

## Указатели

Kato поддерживает указатели для работы с адресами памяти:
...
Чтобы указатель указывал на переменную, используйте оператор `&`:
...
Для разыменования указателя используйте `*ptr*`:
...
Массив можно указать через имя или получить адрес элемента:
...
**ВАЖНО:**
- Указатели работают как в C
- Разыменование через `*ptr*` в выражениях и присваиваниях
- Взятие адреса через `&variable`
- Поддерживаются указатели на все базовые типы
...
## Импорт функций и структур

Kato поддерживает импорт функций и структур из других файлов через .kh (Kato Header) файлы:
...
- Нельзя дублировать имена функций и структур
- Если файл не найден - ошибка
- Синтаксис экспорта структур: `$export struct StructName from file.kato`
