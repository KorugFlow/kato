...
class KatoWarning:
    ...
    def format_warning(self):
        ...
        warning_msg = f"\n{YELLOW}{'='*60}{RESET}\n"
        warning_msg += f"{BOLD}{YELLOW}  Kato Warning{RESET}\n"
        warning_msg += f"{YELLOW}{'='*60}{RESET}\n\n"
        warning_msg += f"{BOLD}  {self.message}{RESET}\n"
        warning_msg += f"{GRAY}  at line {self.line}, column {self.column}{RESET}\n\n"
        ...
        if self.source_code:
            ...
            if 0 < self.line <= len(lines):
                ...
                warning_msg += f"{CYAN}{'─'*60}{RESET}\n"
                
                if self.line > 1:
                    warning_msg += f"{GRAY}  {self.line - 1:4d} | {lines[self.line - 2]}{RESET}\n"
                
                warning_msg += f"{YELLOW}  {self.line:4d} | {error_line}{RESET}\n"
                warning_msg += f"{YELLOW}       | {' ' * (self.column - 1)}^{RESET}\n"
                
                if self.line < len(lines):
                    warning_msg += f"{GRAY}  {self.line + 1:4d} | {lines[self.line]}{RESET}\n"
